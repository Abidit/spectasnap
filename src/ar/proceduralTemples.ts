 ...
 *   0. Hinge        (0, 0, 0)
 *   1. Straight run  at ~40% length — extends backward (-Z = behind the head)
 ...
 *
 * Coordinate system: +Y up, +Z toward camera (face front), -Z behind head.
 * No base rotation is applied — model space matches world space directly.
 */
function buildTempleCurve(
  ...
  // In model space +X = viewer's left (mirrored video), -X = viewer's right.
  const sx = side === 'left' ? 1 : -1;

  // Small lateral offset so the arm angles slightly outward from the hinge.
  const lateralSpread = templeLength * 0.12;

  // Temples extend in -Z (behind the head, toward the ears). This is the
  // correct direction for temple arms to be hidden by the face occluder
  // and appear to wrap around the head. No base rotation compensation needed.
